...
function writeFile(folder, templates) {
    for (var tplName in templates) {
        ...
        try {
            ...
            console.log(chalk.yellow('You already have a file named: ') + chalk.bold.white(tplName));
        }
        catch (e) {
            if (e.code === 'ENOENT') {
...
function registerItem(name, isComponent) {
    ...
    if (moduleFile) {
        ...
        if (content.indexOf(itemName.pascal) > -1) {
            console.log(chalk.yellow.bold(itemName.pascal + ' has already been registered!'));
            return;
        }
