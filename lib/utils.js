...
function registerItem(name, isComponent, includeStylesheet) {
    ...
    if (moduleFile) {
        ...
        if (includeStylesheet && stylesheet) {
            var importStylesheetString = "@import './" + itemName.kebab + "/" + itemName.kebab + ".scss';";
            var scss = fs.readFileSync(path.join(process.cwd(), stylesheet), 'utf8');
            if (scss.indexOf(itemName.kebab) > -1) {
                console.log(chalk.yellow(itemName.kebab + '.scss' + chalk.white(' has already been registered in ' + chalk.cyan.bold(stylesheet) + '!')));
            } else {
                var lines = scss.split(/\n/g);
                lines.splice(0, 0, importStylesheetString);
                scss = lines.join('\n');
